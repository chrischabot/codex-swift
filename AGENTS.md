...
> so Claude Code and other agents read the same notes.

## Upstream sync (codex-rs)

codex-swift is a port of [`openai/codex`](https://github.com/openai/codex). Track
where we are relative to upstream — the commit we last caught up to and what was
folded in — in [`UPSTREAM_SYNC_LOG.md`](UPSTREAM_SYNC_LOG.md). Any in-flight
catchup has a work breakdown in [`codex-catchup.md`](codex-catchup.md). The
wire-surface oracle is pinned in
[`tools/conformance/PINNED_REV`](tools/conformance/PINNED_REV); keep it in step
with the latest **completed** sync-log entry.
