...
-- sqlite-vec virtual table; dim=1536 store width
...
**Phase 2 — Bulk markdown import.** Initial `codex-memory import-markdown` is implemented for local markdown roots: offline dry-run manifest, realpath/symlink guards, deterministic source URIs, idempotent delete-then-process replacement, same-SHA partial-row repair, per-file state persistence, and summary/JSON reports. Seed fixtures: `/Users/chabotc/Projects/agentwiki/data/agentwiki/markdown` (4,865 markdown files discovered) and `/Users/chabotc/Projects/devrel-almanac/devrel` (102 markdown files discovered). Current warm release dry-run over both roots discovers 4,967 files in ~1.85s. Remaining Phase-2 proof work: failure-injection/kill-9 restart tests, full-corpus real import timing, and progress reporting during long imports.

**Phase 3 — MemoryIngest connectors.** RSS/arXiv/GitHub/newsletter/company-news/personal-site fetchers, dedupe, rollout-JSONL archive, sleep/wake handling on macOS via `IORegisterForSystemPower`, SIGUSR2 fallback on Linux. Acceptance: 24h soak ingesting at ≥ 200 docs/day with zero ring-drops and crash-consistent recovery proven by a kill-9 fuzzer.

**Phase 4 — MemoryProcess + MemoryScore.** Contextualisation, extraction (typed JSON schema validated against `ExtractionSchema`), embedding, four-signal scoring, ego-betweenness nightly job. Acceptance: end-to-end on the Agent Wiki + DevRel seed corpora completes in ≤ 4 hours on M3 Max 48 GB; gate score distribution AUC ≥ 0.85 against user stars or a held-out labelled set.

**Phase 5 — Wiki compiler/linter.** Initial `codex-memory wiki-compile` and `wiki-lint` are implemented. The compiler emits deterministic source pages, entity pages, graph-edge claim pages, `_digests/agent-digest.json`, and `_digests/compile-manifest.json`; preserves `codex-wiki:human` edit blocks; refuses vault symlink overwrites; and can explicitly `--index` compiled pages through staged replacement so old FTS/vector rows are purged after successful reprocessing. The linter reuses the markdown import scanner, catches missing titles, duplicate titles/content, broken links, bad front matter, skipped unsafe files, uncompiled sources, stale/missing digests, orphan entities, and SQLite index-health failures. Current warm release lint over the two seed roots checks 4,967 files in ~2.75s and reports 0 errors / 49 warnings against an empty temp store. Remaining Phase-5 work is the real durable claim schema, extraction completion markers, synthesis/dashboard pages, contradiction/staleness reasoning over claim records, and full seed-corpus compile/lint baselines.

**Phase 6 — MemoryRetrieve + MemoryMCP + MLX BGE reranker.** Hybrid search (FTS5 + sqlite-vec + RRF), persona lenses, the MCP toolset, and initial local `BAAI/bge-reranker-v2-m3` cross-encoder support are implemented. `MemoryInfer` now includes a Roberta/BERT sequence-classification wrapper over `MLXEmbedders.Bert`, a custom `EmbedderModelFactory` registry for `xlm-roberta` reranker configs, query/candidate pair tokenization, lazy BGE loading, and cosine fallback when the local cross-encoder cannot load or MLX is disabled. Remaining Phase-6 work is live model-load benchmarking, labelled seed-corpus NDCG@10 evaluation, and pressure-driven reranker unload/reload polish.

**Phase 7 — Production tools.** Initial `wiki_brief`, `wiki_compare`, `wiki_angle`, and `wiki_pmfit` are implemented in `MemoryMCP` and wired into `MemoryToolset`. They are intentionally lexical-only in v1, backed by `MemoryStore.searchLexical` + `chunkEvidence`: no embedding calls, no model calls, no hidden cloud spend, `retrieval.cloud_spend_usd = 0`, every generated point includes chunk citation IDs, each payload includes confidence, "what would change my mind", and `uncited_claims: []`, and no-evidence cases return `insufficient_evidence` instead of invented analysis. Remaining work is to upgrade these tools from mechanical evidence layouts to synthesis over durable claim records, topic packs, and seed-corpus eval prompts once Phase 5's real claim schema lands.

**Phase 8 — BrainGate + TwitterAPI.io.** Monthly USD ceiling, single-flight escalations, insight cards. Acceptance: chaos test firing 1,000 high-score chunks is rate-limited to the configured monthly cap; `spend` ledger reconciles to within $0.01 of the actual OpenAI usage record.

**Phase 9 — Hardening + soak.** 7-day soak, sleep/wake/restart fuzz, memory-pressure injection via `DispatchSourceMemoryPressure`. Acceptance: zero extractor evictions, zero ring-drops, p99 MCP tool latency ≤ 250 ms.

### 11 — Cost analysis

Workload assumptions: the first local seed is 4,967 discovered markdown files
(4,865 Agent Wiki + 102 DevRel), growing toward an 8,000-doc working corpus;
200 docs/day steady-state; 10 chunks/doc avg; ~600 tokens/chunk
(contextualised); ~800 tokens of context-window for the situating prompt per
chunk; gate accept rate ~3% of chunks → **~60 insight-card calls/day**, each
averaging 6K input / 1.5K output tokens to GPT-5.5; TwitterAPI.io budget of 20k
tweets/day pull.
...
- **MLX Swift embedding/reranker maturity.** `MLXEmbedders` 3.31.3 ships Bert/NomicBert/Qwen3 only; no ModernBERT, no cross-encoder head. codex-swift now carries an initial `bge-reranker-v2-m3` wrapper in `MemoryInfer` (Roberta/BERT encoder plus sequence-classification head). Fallback remains cosine/RRF when MLX or the local reranker weights are unavailable; richer labelled evaluation and live load benchmarks are still required.
...
2. **Harden bulk markdown import against crash/restart.** The Agent Wiki and DevRel roots now dry-run deterministically and the importer is idempotent on small fixtures. Next proof is failure-injection/kill-9 restart recovery plus full-corpus real import timing.
3. **Build the wiki compiler/linter before production tools.** `wiki_compare`, `wiki_angle`, and `wiki_pmfit` are only valuable once source pages, claims, contradictions, backlinks, and `agent-digest.json` exist with stable lint output.
4. **Keep the local BGE-reranker-v2-m3 wrapper in `MemoryInfer` rather than waiting on upstream MLXEmbedders.** ModernBERT and cross-encoder heads are not in `mlx-swift-lm` 3.31.3 and are not part of the current package surface; the local wrapper keeps the project moving while remaining removable if upstream ships an official head.
5. **Ship production tools as cited workflows, not generators.** Each output should include citations, novelty rationale, confidence, and "what would change my mind"; seed-corpus evals should reject uncited claims.
6. **Vendor `sqlite-vec.c` from the v0.1.9 stable tag, not the v0.1.10-alpha line.** The DiskANN alpha is promising but not load-bearing-worthy; v0.1.9 with brute-force `vec0` is the correct production choice for 80k–200k chunks. Track the alpha on a side branch and migrate when GA ships.
7. **Pin the extractor to Qwen3-30B-A3B-MLX-4bit and the embedder to Nomic-Embed-Text-v1.5.** Nomic is natively 768-dim; the production store width is 1536 so local vectors are padded while remote `text-embedding-3-small` can emit 1536 directly. Both models are stable, well-benchmarked, fit comfortably under the 48 GB budget, and have direct mlx-community releases. Resist chasing newer Qwen3.5/3.6 weights until the M5 Max upgrade.
8. **Treat the monthly USD ceiling as a hard SLO, not a soft target.** Default $40/mo for personal use, configurable upward; expose it in an MCP `spend_status` micro-tool so a human can see the curve.
