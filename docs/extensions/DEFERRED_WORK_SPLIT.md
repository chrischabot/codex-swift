...
| 4 | **Wiki tool/recall feature-gate asymmetry.** Provider tools registered on `[memory].provider` alone while recall was gated on `extensions`, leaking e.g. wiki tools into an extensions-off session (tool-list divergence from core). | **Superseded by the mem0-default design:** personal memory is no longer gated by general `extensions`; selected provider recall/capture and tools install together. General manifests remain feature-gated. | `HarnessCoreTests/ExtensionsTests/testInstallAddonsInstallsMemoryProviderWithoutExtensionsFeature` plus provider-selection tests |
