...
- **Signed media tokens** (`MediaToken.swift`). A `/media/:token` URL is a stateless capability: the file path *relative to the media root* plus an expiry is HMAC-SHA256-signed under a **per-launch random key** (so links die on restart). Verification is purely cryptographic — no session lookup — so a plain `<img>` or `<video>` tag just works. `..` is rejected on both sign and verify, and the route re-resolves the path under the media root. `Range`/206 is supported so audio/video can seek.
