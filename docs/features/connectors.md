...
- **Google Workspace tool suite — built.** A discovery-driven `google_api` universal tool plus ergonomic typed READ helpers (`gmail_search`, `drive_get`, `calendar_agenda`) packaged as a `ToolPack`, gated on `[features].google` + a `[connectors.google]` table, with a service→host containment allowlist and write/destructive verbs forced through the approval gate. (Typed write helpers + static scope-pruning are follow-ons; reads fail closed on a missing scope.)
