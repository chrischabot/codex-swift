...
**Built, tested, and wired — now with a LIVE provider and signed delivery.** The `Media` module (`MediaTaskLedger` submit/poll lifecycle, idempotency dedup, decoupled-and-bounded delivery retry, `FileMediaStore` durable atomic store, `MediaPoller`, `MediaConfig`, the deny-default provider factory, `MediaWiring`), the approval-gated `media_generate` tool, and the codexd/codex-session wiring (in-process eager ledger + poller, spawned-worker self-prune for async providers) are all live. Severe tests cover config deny-default + key gating + `$CODEX_HOME` expansion + the `CODEX_FEATURE_MEDIA` env, the stub write-under-root, file-store round-trip + crash recovery, the poller driving a recovered queued task to done+delivered, transient-delivery retry + bound, the empty-`deliver_to` ungated-and-not-pushed normalization, bounded terminal-task retention, and the async-provider-in-spawned-mode refusal.

**Live provider (#3) — `OpenAIImagesProvider`.** Set `[media] provider = "openai"` + `api_key_env = "OPENAI_API_KEY"`: it POSTs `gpt-image-1` to `/v1/images/generations`, decodes the inline base64 PNG, and writes `image-<uuid>.png` under the media root — `.inline` (no second-URL download). It's in `inlineProviders`, so `requiresPoller == false` and it works in BOTH in-process and spawned-worker modes. A fresh adversarial review hardened the transport: redirects are REFUSED (the bearer is never replayed to a 3xx target — the real exfil surface), the streamed response is hard-capped (no multi-GB OOM), and transport errors are redacted (no raw `URLError` userInfo in a persisted reason). The key only ever lives in the request header. 46 `MediaTests` (stubbed HTTP seam, no key) + a gated live e2e that XCTSkips cleanly without `OPENAI_API_KEY`.

**Signed URL delivery (#4).** When the [web gateway](web-gateway.md) is up, `MediaGlue.push` mints a stable, signed `<base>/media/<token>` URL (HMAC, TTL, path-contained) instead of a local path — surviving a daemon restart when `CODEXKIT_WEB_PERSIST_MEDIA_SIGNER=1` (an atomic 0600 key store). It degrades to the local path when there's no gateway signer, the asset is outside the media root, or the bind is a wildcard with no `CODEXKIT_WEB_PUBLIC_BASE_URL` (so it never hands out an unreachable `0.0.0.0` link).

**Next:** a truly async provider (fal — `.queued` + real poller), and the `pdf_read`/`transcribe` inbound tools over the sandboxed decoder.
