...
/// Pure-Swift SHA-256 (FIPS 180-4), shared across modules that must avoid an
/// external Crypto dependency and build identically on Linux/macOS. Named
/// distinctly from `Auth.SHA256` so the two can coexist without ambiguity.
public enum Hashing {
    ...
    public static func sha256Hex(_ s: String) -> String {
        sha256(Array(s.utf8)).map { String(format: "%02x", $0) }.joined()
    }
}
